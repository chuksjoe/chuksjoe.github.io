...
$(document).ready( function(){
	...
	$(".section-head").on("click", function(){
		if($(this).next().css('display') == 'none'){
			...
			if($(this).children("div.sec-title").text() == "GALLERY"){
				//alert("You now viewing my GALLERY!");
				//imgSlideTimer = setInterval(autoSlideRight, 1500);
			}
			$(this).next().slideDown('slow');
			$(this).children("div.sec-title").addClass("active");
			
		}else{
			$(this).next().slideUp();
			$(this).children("div.sec-title").removeClass("active");
	...
	function scrollImgLeft(){
		if(w < -5){
			w += imgWidth;
			$("#image-holder").css({transform: "translate(" + w + "px, 0px)", transition: ".5s ease-in-out"});
		}
	}
	...
	$(".skill").each(function(){
		...
		var level = $(this).find("div.progress-level").text();
		$(this).find("div.progress-level").css({width: level, background: "rgb(" + r + "," + g + ",100)"});
	});
